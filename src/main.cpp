...
/* COMECANDO A EXECUCAO DO JOGO*/
    ...
    if(inicializaJogo()){
        ...
        //al_draw_scaled_bitmap(map, 50*x, 20*y, res_x_comp, res_y_comp, 0, 0, res_x_comp*(res_x_comp/1920.0)*5, res_y_comp*(res_y_comp/1080.0)*5, 0);
        //al_draw_scaled_bitmap(player_f1, 0, 0, res_x_player, res_y_player, 0, 0, res_x_player*(res_x_player/16.0)*0.01, res_y_player*(res_y_comp/32.0)*0.01 , 0);
        al_draw_bitmap(map,0,0,0);
        al_draw_bitmap(player_f1,EIXO_X_PLAYER,EIXO_Y_PLAYER,0); //desenhado o personagem na tela
        al_flip_display();
...
void redesenhar(int posx, int posy, ALLEGRO_BITMAP *img1, ALLEGRO_BITMAP *img2, long long int c){
    al_draw_bitmap(map,0,0,0);
    if(c%2==0)al_draw_bitmap(img1,posx,posy,0); //desenhado o personagem na tela
    else al_draw_bitmap(img2,posx,posy,0);
    al_flip_display();
}
